...
class Trainer(TrainerBase):
        ...
        for task_idx, task in enumerate(self.task_list[latest_task_idx+1:]):  # for each task, train for several epochs
            ...
            if args.memory:
                if task_idx != latest_task_idx + 1:
                    # print(f"Current task_id is {task_idx}\n")
                    # if task_idx == 0:
                    #     each_memory = self.M
                    # else:
            ...
            # ========= Testing =========     # Comment this when you resume through checkpoint to avoid test error on all task
            # self.test(task)
            self.test(task, task_idx)
            if self.composition_test_cate in Category_splits:
                self.test(task, task_idx, comp=True)
                self.test_nocomp(task, task_idx)
    ...
    def test(self, task, task_idx, comp=False):
        # Test Set
        os.makedirs(self.args.output, exist_ok=True)
    ...
    def test_nocomp(self, task, task_idx):
        # Test Set
        os.makedirs(self.args.output, exist_ok=True)
        # last_path = os.path.join(self.args.output, task + '_LAST')
        # self.load(last_path)
...
def main_worker(gpu, args):
    ...
    if args.now_train:
        ...
        try:
            ...
            print(trainer.result_matrix_comp['q_causal'])
            acc = 0
            for key in trainer.result_matrix_comp['q_causal']:
                acc += trainer.result_matrix_comp['q_causal'][key]
