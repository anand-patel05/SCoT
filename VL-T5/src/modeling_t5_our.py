        ...
        if self.config.num_layers > 0:
            ...
            for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
                ...
                layer_outputs = layer_module(
                    ...
                    position_bias=position_bias, # 除了text, vision部分始终是0
...
class VLT5(T5ForConditionalGeneration):
    ...
    def update_prototype(
        ...
        current_Q_prototype = current_Q_prototype.detach()
        current_V_prototype = current_V_prototype.detach()
        current_num_Q = current_num_Q.detach()
        current_num_V = current_num_V.detach()
        ...
        if current_task_id not in self.Q_task_cur_proto:
            ...
            if current_task_id != 0:
                ...
                if current_task_id not in self.Q_task_mem_proto:
                    ...
                    self.Q_task_mem_proto[current_task_id] = (
                        proto_alpha * self.Q_task_mem_proto[current_task_id].detach()
                        + (1 - proto_alpha) * current_Q_prototype_mem
                    ).detach()

                self.Q_prototype = self.Q_task_mem_proto[current_task_id].detach().clone()
                self.Q_prototype[current_task_id] = (
                    self.Q_task_cur_proto[current_task_id][current_task_id].detach()
                )
            else:
                self.Q_prototype = self.Q_task_cur_proto[current_task_id].detach()

            self.V_prototype = (
                proto_beta * self.V_prototype.detach()
                + (1 - proto_beta) * current_V_prototype
            ).detach()
    ...
    def calculate_current_prototype(self, fc_hidden_Q, labels):
        ...
        div_item_ = torch.sum(labels, dim=0).unsqueeze(1).repeat(
            1, fc_hidden_Q.shape[-1]
        )
        ones = torch.ones_like(div_item_)
    ...
    def forward(
        ...
        **kwargs,
        ...
        B, L = attention_mask.size()
        needed_vis_len = hidden_states.size(1) - L

        if vis_attention_mask is None:
            vis_attention_mask = attention_mask.new_ones(B, needed_vis_len)
        elif vis_attention_mask.size(1) < needed_vis_len:
            extra_len = needed_vis_len - vis_attention_mask.size(1)
            extra_mask = attention_mask.new_ones(B, extra_len)
            vis_attention_mask = torch.cat([vis_attention_mask, extra_mask], dim=1)
