...
class VQAFineTuneDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if len(scores) == B:
            ...
            batch_entry['scores'] = torch.ones(B, dtype=torch.float)  # default 1.0 for classifier mode
...
class VQADataset:
    ...
    def __init__(self, splits: str, verbose=True):
        ...
        # if verbose:
        #     print("\nLen of id2datum_gt = ", len(self.id2datum_gt))
        #     print("\nLen of id2datum = ", len(self.id2datum))
...
# --------------------- commenting just for experiment -- above code is original -------------------------- #
...
class VQAEvaluator:
    ...
    def evaluate(self, quesid2ans: dict):
        ...
        for quesid, ans in quesid2ans.items():
            ...
            # Ensure 'label' key exists before accessing
            if 'label' in datum:
                label = datum['label']
                if ans in label:
                    score += label[ans]
            else:
                # print(f"Warning: Missing 'label' for question_id {quesid}")
                continue  # Skip this entry if 'label' is missing

        return score / len(quesid2ans) if len(quesid2ans) > 0 else 0.0
