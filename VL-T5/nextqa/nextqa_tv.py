...
# Import Task Vector
from task_vector import TaskVector, consolidate_layer

from param import parse_args
from Question_type_nextqa import All_Q
from Question_type_nextqa import All_V as Category_splits
from Question_type_nextqa import show_results_matrix, evaluate_metric, random_dic, save_results_matrix # plot_result
...
class Trainer(TrainerBase):
    def __init__(self, args, All_Q, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        #         split=args.test, mode='test', batch_size=args.valid_batch_size,
        #         distributed=args.distributed, gpu=args.gpu,
        #         workers=4,
        #         topk=args.valid_topk,
        #         task=task,
        #     )
        #     self.test_loader_dict_all[task] = test_loader                 # defining outside for testing all data everytime
        ...
        for task_idx, task in enumerate(self.task_list):
            ...
            if args.memory:
                if task_idx != 0:
                    ...
                    # idx-1 memory
                    sample_list_file = os.path.join('/home/vis-comp/24m2119/HypVQACL/datasets/nextqa/Partition_Q/train_'+ f'{self.task_list[task_idx - 1]}.csv')
            ...
            task_vector = TaskVector(pretrained_checkpoint=initial_model_wt, finetuned_checkpoint=finetuned_ckpt_path)
            ...
            # Test Set
            # ========= Testing =========
            self.test(task, -1, task_idx)
            if self.composition_test_cate in Category_splits:
                self.test(task, -1, task_idx, comp=True)
                self.test_nocomp(task, -1, task_idx)
    ...
    def test(self, task, latest_task_idx, task_idx, comp=False):
        # Test Set
        os.makedirs(self.args.output, exist_ok=True)
    ...
    def test_nocomp(self, task, latest_task_idx, task_idx):
        # Test Set
...
if __name__ == "__main__":
    ...
    print(f"[RUN CONFIG] seed={args.seed}, epochs={args.epochs}, batch_size={args.batch_size}, output={args.output}")
