...
# from metrix import get_wups # missing
...
import nltk
nltk.data.path.append("/home/vis-comp/24m2119/VQACL/VL-T5/nextqa/nltk_data")
...
stopwords = load_file('/home/vis-comp/24m2119/VQACL/VL-T5/nextqa/stopwords.txt')
...
class VQAFineTuneDataset(Dataset):
    def __init__(self, split, Examplar_set, raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='DO', cates=[0,1,2]):
        ...
        sample_list_file = os.path.join('/home/vis-comp/24m2119/VQACL/datasets/nextqa/Partition_Q', '{}_{}.csv'.format(mode, task))
        ...
        vid_feat_file = os.path.join('/home/vis-comp/24m2119/VQACL/datasets/nextqa', 'vid_feat/app_mot_{}.h5'.format(mode))
...
class VQADataset:
    ...
    def __init__(self, splits: str, verbose=True):
        ...
        sample_list_file = os.path.join('/home/vis-comp/24m2119/VQACL/datasets/nextqa', 'train.csv')
        train2014_data = load_file(sample_list_file)
        # sample_list_file = os.path.join('/data/zhangxi/nextqa', 'val.csv')
        sample_list_file = os.path.join('/home/vis-comp/24m2119/VQACL/datasets/nextqa', 'val.csv')
        ...
        for split in self.splits:
            ...
            sample_list = load_file(os.path.join('/home/vis-comp/24m2119/VQACL/datasets/nextqa', split+'.csv'))
...
class VQAEvaluator:
    ...
    def remove_stop(self, sentence):
        # Ensure sentence is a str, not bytes
    ...
    def evaluate_raw(self, quesid2ans: dict, is_topk_optimal=None):
        ...
        self.evalQuesType = {}
        self.evalAnsType  = {}
        self.add_ref = load_file('/home/vis-comp/24m2119/VQACL/datasets/nextqa/add_reference_answer_test.json')
        ...
        for quesId, resAns in tqdm(quesid2ans.items(), total=len(quesid2ans), ncols=80):
            ...
            if quesId not in self.dataset.id2datum:
                print("Missing quesId:", quesId)
                print("Available example key:", next(iter(self.dataset.id2datum.keys())))
                break

            datum = self.dataset.id2datum[quesId]

            # if quesId in self.dataset.id2datum:
            #     datum = self.dataset.id2datum[quesId]
            # elif quesId in self.dataset.id2datum_gt:
            #     datum = self.dataset.id2datum_gt[quesId]
            # else:
            #     # Optional: print and skip instead of crashing
            #     # print("Missing quesId:", quesId)
            #     continue
